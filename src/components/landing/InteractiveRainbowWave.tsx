...
const InteractiveRainbowWave = ({
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      amplitudeRef.current +=
        (targetAmplitudeRef.current - amplitudeRef.current) * 0.1;
      ...
      for (let x = 0; x <= width; x += 2) {
        ...
        const localizedAmp =
          amplitudeRef.current * (1 + Math.max(0, mouseInfluence) * 0.5);

        const t = progressRef.current;

        // Wave 1: Main vocal line
        const y1 = Math.sin(x * 0.03 + t) + Math.sin(x * 0.08 - t * 0.5) * 0.5;

        // Wave 2: Harmonic echo (ghost line)
        const y2 = Math.cos(x * 0.04 - t * 0.8) + Math.sin(x * 0.1 + t) * 0.3;
  ...
  return (
    <div
      ...
      className={cn(
        "w-full h-24 relative flex items-center justify-center cursor-crosshair touch-none select-none",
        className
      )}
    >
      <svg
      ...
      >
        <defs>
          ...
          >
            <stop
              offset="0%"
              stopColor={currentLanguage.from}
              stopOpacity="0.2"
            />
            <stop
              offset="50%"
              stopColor={currentLanguage.via}
              stopOpacity="0.2"
            />
            <stop
              offset="100%"
              stopColor={currentLanguage.to}
              stopOpacity="0.2"
            />
          </motion.linearGradient>
        </defs>
        ...
        <path
          ...
          fill="none"
          stroke={
            lineColor ? lineColor : `url(#ghostGradient-${currentLanguage.id})`
          }
          strokeWidth="2"
          strokeLinecap="round"
        ...
        <path
          ...
          stroke={
            lineColor
              ? lineColor
              : `url(#rainbowGradient-${currentLanguage.id})`
          }
          strokeWidth="3"
          strokeLinecap="round"
          strokeLinejoin="round"
